...
void SeqListInit(SeqList* seqListPtr)
{
	...
	{
		typedata i;
		while (fread(&i, sizeof(typedata), 1, p))
		{
...
void SeqListDestroy(SeqList* seqListPtr)
{
	...
	{
		size_t count = 0;
		count = fwrite(seqListPtr->ptr, sizeof(typedata), seqListPtr->datacount, p);
...
static void MemoryBlockExpand(SeqList* seqListPtr)
{
	...
	typedata* p = Realloc(typedata, seqListPtr->ptr, newcapacity);
	assert(p);
	seqListPtr->ptr = p;
